...
public:
    ...
    void hapusbarang(int kode) {
        for (int i = 0; i < jumlahbarang; ++i) {
            if (inventaris[i].kode == kode) {
                ...
                jumlahbarang--; // Corrected decrement operation
    ...
    void ubahbarang(int kode, string nama, int jumlah, string satuan, double harga) {
        for (int i = 0; i < jumlahbarang; i++) {
            if (inventaris[i].kode == kode) {
                inventaris[i].nama = nama; // Corrected assignment operator
                inventaris[i].jumlah = jumlah; // Corrected assignment operator
                inventaris[i].satuan = satuan; // Corrected assignment operator
                inventaris[i].harga = harga; // Corrected assignment operator
    ...
    void tampilkaninventaris() {
        ...
        for (int i = 0; i < jumlahbarang; ++i) {
            cout << "Kode: " << inventaris[i].kode
                << ", Nama: " << inventaris[i].nama
                << ", Jumlah: " << inventaris[i].jumlah
                << ", Satuan: " << inventaris[i].satuan
    ...
    void bubblesort() {
        for (int i = 0; i < jumlahbarang - 1; ++i) {
            for (int j = 0; j < jumlahbarang - i - 1; ++j) {
                if (inventaris[j].kode > inventaris[j + 1].kode) { // Corrected indexing
                    baranginventaris temp = inventaris[j];
                    inventaris[j] = inventaris[j + 1]; // Corrected indexing
                    inventaris[j + 1] = temp;
                }
            }
        }
...
int main() {
    ...
    } while (pilihan != 7); // Moved this line outside the switch statement
